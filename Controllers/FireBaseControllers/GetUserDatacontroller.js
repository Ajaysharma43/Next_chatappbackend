...
export const GetUserDetails = async (req, res, next) => {
    try {
        ...
        const UserHiddenPostsData = await pool.query(`
                        SELECT 
                        iu.id AS image_id,
                        iu.name,
                        iu.image_url,
        ...
        res.status(200).json({ message: "user data is successfully fetched", success: true, UserDetails: UserDetails.rows, UserFollowerData: UserFollowerData.rows, UserFollowingData: UserFollowingData.rows, UserImagesUploadData: UserImagesUploadData.rows, UserHiddenPostsData: UserHiddenPostsData.rows })
    } catch (error) {
